...
Shader "Dude" {
	...
	half4 fragAddSub (v2f i) : SV_Target {
		half4 toAdd = tex2D(_Overlay, i.uv[1]) * _Intensity;
		float centerDist = length( float2(.5,.5) - i.uv[0] ) / length(float2(.5, .5) - float2(0,0));
		float minus =1- centerDist;
		minus = minus*minus*minus;
		//minus = 1 - minus;
		float2 newUV = float2(i.uv[1].x + minus*sin(centerDist * 1),
			                  i.uv[1].y + minus*sin(centerDist * 1 ));
		return tex2D(_MainTex, newUV) + toAdd;
	}
